...
LuaModuleManager::LuaModuleManager(LuaManager& manager)
    : manager_(manager) {
}
...
bool LuaModuleManager::Register(std::unique_ptr<LuaModule> module) {
    if (!module || Find(module->Name()) != nullptr) {
        return false;
    }

    if (!module->Initialize(manager_)) {
        return false;
    }
...
bool LuaModuleManager::Remove(const std::string_view name) {
    const auto it = std::ranges::find_if(
        modules_,
        [name](const std::unique_ptr<LuaModule>& module) {
...
void LuaModuleManager::Shutdown() noexcept {
...
std::size_t LuaModuleManager::Size() const noexcept {
