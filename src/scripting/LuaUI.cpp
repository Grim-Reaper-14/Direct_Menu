...
std::uint64_t LuaUI::AddText(std::string owner, std::string text) {
    ...
    widgets_.push_back(Widget{.id=id,.owner=std::move(owner),.type=WidgetType::Text,.label=std::move(text)});
    return id;
}

std::uint64_t LuaUI::AddButton(std::string owner, std::string label, sol::protected_function callback) {
    if (!callback.valid()) return 0;
    const std::uint64_t id = nextId_++;
    widgets_.push_back(Widget{.id=id,.owner=std::move(owner),.type=WidgetType::Button,.label=std::move(label),.callback=std::move(callback)});
    return id;
}

std::uint64_t LuaUI::AddCheckbox(std::string owner, std::string label, const bool initialValue, sol::protected_function callback) {
    const std::uint64_t id = nextId_++;
    widgets_.push_back(Widget{.id=id,.owner=std::move(owner),.type=WidgetType::Checkbox,.label=std::move(label),.value=initialValue,.callback=std::move(callback)});
    return id;
}

bool LuaUI::Remove(const std::uint64_t id) {
    const auto oldSize = widgets_.size();
    std::erase_if(widgets_, [id](const Widget& widget) { return widget.id == id; });
    return widgets_.size() != oldSize;
}

void LuaUI::RemoveByOwner(const std::string_view owner) {
    std::erase_if(widgets_, [owner](const Widget& widget) { return widget.owner == owner; });
}
