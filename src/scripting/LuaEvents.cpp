...
LuaEvents::LuaEvents(logging::LoggerApi& logger)
    : logger_(logger) {
}
...
std::uint64_t LuaEvents::Subscribe(
    ...
    if (eventName.empty() || !callback.valid()) {
        return 0;
    }
...
void LuaEvents::RemoveByOwner(const std::string_view owner) {
    for (auto& [name, handlers] : handlers_) {
        std::erase_if(handlers, [owner](const Handler& handler) {
