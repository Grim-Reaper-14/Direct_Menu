...
LuaTimerManager::LuaTimerManager(logging::LoggerApi& logger, OwnerChangedCallback ownerChanged)
...
std::uint64_t LuaTimerManager::Every(
    ...
    std::string owner,
    sol::protected_function callback) {
    return Add(std::max(interval, std::chrono::milliseconds{1}), true, std::move(owner), std::move(callback));
}
...
bool LuaTimerManager::Cancel(const std::uint64_t id) {
    const auto iterator = std::ranges::find_if(timers_, [id](const Timer& timer) { return timer.id == id; });
    if (iterator == timers_.end()) return false;
    iterator->cancelled = true;
    return true;
}

void LuaTimerManager::RemoveByOwner(const std::string_view owner) {
    std::erase_if(timers_, [owner](const Timer& timer) { return timer.owner == owner; });
}

void LuaTimerManager::Clear() { timers_.clear(); }

void LuaTimerManager::Update() {
    const auto now = std::chrono::steady_clock::now();
    for (Timer& timer : timers_) {
        if (timer.cancelled || now < timer.next) continue;

        if (ownerChanged_) ownerChanged_(timer.owner);
        const sol::protected_function_result result = timer.callback(timer.id);
        if (ownerChanged_) ownerChanged_({});

        if (!result.valid()) {
            const sol::error error = result;
            logger_.Error(std::format("Lua timer {} owned by '{}' failed: {}", timer.id, timer.owner, error.what()));
            timer.cancelled = true;
            continue;
        }

        if (timer.repeating) timer.next = now + timer.interval;
