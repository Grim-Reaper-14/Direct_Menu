...
inline NativeRegistrationTable::NativeRegistrationTable(
    const std::uintptr_t tableAddress) noexcept
    : tableAddress_(tableAddress) {}

inline void NativeRegistrationTable::SetAddress(
    const std::uintptr_t tableAddress) noexcept {
...
inline std::uintptr_t NativeRegistrationTable::Address() const noexcept {
    std::scoped_lock lock(mutex_);
    return tableAddress_;
}

inline bool NativeRegistrationTable::IsReady() const noexcept {
    std::scoped_lock lock(mutex_);
    return tableAddress_ != 0 && static_cast<bool>(resolver_);
}

inline void NativeRegistrationTable::SetResolver(Resolver resolver) {
    std::scoped_lock lock(mutex_);
    resolver_ = std::move(resolver);
    cache_.clear();
}

inline void NativeRegistrationTable::ClearResolver() {
    std::scoped_lock lock(mutex_);
    resolver_ = nullptr;
    cache_.clear();
}
...
inline void NativeRegistrationTable::ClearCache() noexcept {
    std::scoped_lock lock(mutex_);
    cache_.clear();
}

inline std::size_t NativeRegistrationTable::CachedHandlerCount() const noexcept {
    std::scoped_lock lock(mutex_);
    return cache_.size();
}

inline NativeInvoker::Backend NativeRegistrationTable::CreateBackend() {
    return [this](const NativeHash hash,
...
inline bool NativeRegistrationTable::Invoke(
    ...
    try {
        ...
        errorMessage = std::string{"Native engine handler threw an exception: "} + exception.what();
        return false;
    } catch (...) {
...
inline void NativeRegistrationTable::EngineCallContext::FixVectors() noexcept {
